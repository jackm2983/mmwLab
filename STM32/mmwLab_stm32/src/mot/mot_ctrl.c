...
uint8_t mot_ctrl_move_axis1_rel(int32_t steps, uint32_t freq_hz)
{
    ...
    // bounds check removed for now, limit switches and calibrated bounds in
    // mot_axis handle the safety. revisit when soft bounds are needed.
...
{
    if (freq_hz == 0 || freq_hz > MOTOR_MAX_SPEED) return 2;
    if (direction && mot_axis1_limit_triggered()) return 3;
...
{
    if (freq_hz == 0 || freq_hz > MOTOR_MAX_SPEED) return 2;
    if (direction && mot_axis2_limit_triggered()) return 3;
...
/* ============================================================================
 * Status Update - call every loop iteration
 *   pulls actual step deltas from the timer and updates axis position
 * ============================================================================ */
